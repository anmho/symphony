...
export async function runStatusWatch(options: WatchOptions): Promise<void> {
  ...
  output.write(renderStatusScreen(snapshot, {
    nowMs: Date.now(),
    port: options.port,
    selectedIndex: 0,
    color: false
...
async function runOpenTuiStatusWatch(options: Required<Pick<WatchOptions, "port" | "intervalMs" | "input" | "output">>): Promise<void> {
  const input = options.input ?? process.stdin;
  const output = options.output ?? process.stdout;
    ...
    if (inputMode !== "normal") {
      ...
      if (key.name === "return" || key.name === "enter") {
        if (inputMode === "command") {
          ...
          if (command === "agents" || command === "agent" || command === "ag") {
            view = "agents";
          } else if (command === "describe" || command === "desc" || command === "d") {
            view = "describe";
          } else if (command === "events" || command === "event" || command === "logs" || command === "l") {
            view = "events";
            ...
            void selectedIssue(options.port, filterText, selectedIndex).then((issue) => issue ? resumeIssue(options.port, issue) : null).then(() => render());
          } else if (command.startsWith("steer ")) {
            const text = commandBuffer.trim().slice("steer ".length);
            void selectedIssue(options.port, filterText, selectedIndex).then((issue) => issue ? queueSteer(options.port, issue, text) : null).then(() => render());
          }
        } else {
...
export function renderStatusScreen(
  ...
  const selectedIndex = clamp(options.selectedIndex, 0, Math.max(rows.length - 1, 0));
...
async function selectedIssue(port: number, filterText: string, selectedIndex: number): Promise<string | null> {
  const snapshot = await fetchDaemonStatus(port);
  if (!snapshot) {
    return null;
  }
  return filteredRows(snapshot, Date.now(), filterText)[selectedIndex]?.issue ?? null;
}
...
function watchRows(snapshot: OrchestratorSnapshot, nowMs: number): WatchRow[] {
  const running = snapshot.running.map((session): WatchRow => ({
    ...
    updated: session.lastCodexTimestamp ? formatDuration(nowMs - session.lastCodexTimestamp) : "-",
  ...
  const completed = snapshot.completed.map((issueId): WatchRow => ({
    kind: "completed",
    issue: issueId,
    age: "-",
    turn: "-",
...
function summarizeCodexMessage(value: string | null): string {
  ...
  try {
    ...
    if (item?.command) {
      return truncate(`${item.type ?? "command"} ${item.status ?? ""}: ${item.command}`.trim(), 180);
    }
    if (item?.text) {
...
function createTheme(enabled: boolean): Theme {
  const paint = (code: string, value: string) => enabled ? `\x1b[${code}m${value}\x1b[0m` : value;
  return {
...
function ansiToStyledText(value: string, opentui: Record<string, unknown>): unknown {
  ...
  while ((match = regex.exec(value)) !== null) {
    if (match.index > index) {
      chunks.push(applyAnsiStyle(value.slice(index, match.index), style, { fg, bold, dim, reverse }));
    }
  ...
  if (index < value.length) {
    chunks.push(applyAnsiStyle(value.slice(index), style, { fg, bold, dim, reverse }));
  }

  return new StyledText(chunks);
}

function ansiStyle(): { color: string | null; bold: boolean; dim: boolean; reverse: boolean } {
  return {
    color: null,
    bold: false,
    dim: false,
