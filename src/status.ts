...
export interface StatusServerControls {
  resumeRateLimitedRuns?: () => Promise<{ resumed: number }> | { resumed: number };
  getEvents?: (query: { issue: string | null; cursor: number | null; limit: number | null }) => AgentWorkEvent[];
...
export function startStatusServer(
  ...
  const server = http.createServer(async (request, response) => {
    ...
    if (url.pathname === "/events" && request.method === "GET" && controls.getEvents) {
    ...
    if (url.pathname === "/control/steer" && request.method === "POST" && controls.queueSteer) {
      ...
      response.writeHead(200, { "content-type": "application/json" });
      response.end(JSON.stringify({ ...result, snapshot: getSnapshot() }, null, 2));
      return;
    }

    if (url.pathname === "/control/resume-issue" && request.method === "POST" && controls.resumeIssue) {
      const body = await readJsonBody(request);
      const issue = typeof body.issue === "string" ? body.issue : "";
      if (!issue) {
        response.writeHead(400, { "content-type": "application/json" });
    ...
    if (url.pathname === "/control/resume-rate-limited" && request.method === "POST" && controls.resumeRateLimitedRuns) {
...
export async function resumeRateLimitedRuns(port = DEFAULT_STATUS_PORT): Promise<{ resumed: number } | null> {
  try {
    const response = await fetch(`http://127.0.0.1:${port}/control/resume-rate-limited`, { method: "POST" });
    if (!response.ok) {
      return null;
    }
...
export async function fetchDaemonEvents(
  port = DEFAULT_STATUS_PORT,
  query: { issue?: string | null; cursor?: number | null; limit?: number | null } = {}
...
export async function resumeIssue(
  ...
  try {
    const response = await fetch(`http://127.0.0.1:${port}/control/resume-issue`, {
      method: "POST",
      headers: { "content-type": "application/json" },
